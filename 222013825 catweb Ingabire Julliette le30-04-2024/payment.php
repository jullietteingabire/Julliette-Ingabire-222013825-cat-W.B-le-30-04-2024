...
<form action="payment.php" method="POST" style="background-color: darkorange; align-items: center; width: 300px; height: 290px;"> <br><br><br>
        <label>Enter Payment_id:</label><br>
        <input type="number" name="Payment_id" required><br>
        <label>Enter Amaunt:</label><br>
        <input type="number" name="Amaunt" required><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("INSERT INTO payment(Payment_id,Amaunt,PaymentDate,PaymentMethod,Customer_id) VALUES (?, ?, ?, ?,?)");
    $stmt->bind_param("sssss",$Payment_id,  $Amaunt,$PaymentDate, $PaymentMethod,$Customer_id );
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                echo "<tr><td>" . $row["Payment_id"] .
                     ...
                     "</td><td><a href='Paymentupdate.php?updatePayment_id=". $row['Payment_id']."'>UPDATE</a></td><td><a href='Paymentdelete.php?deletePayment_id=".$row['Payment_id']."'>DELETE</a></td></tr>";       
            }
        } else {
